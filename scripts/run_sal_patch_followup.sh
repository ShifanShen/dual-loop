...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <local_model_path> [max_problems] [model_style]"
  echo "Example: $0 /models/Qwen2.5-Coder-7B-Instruct 50 CodeQwenInstruct"
  exit 1
fi
...
echo "[1/2] Running SAL patch loop_b_only follow-up..."
CUDA_VISIBLE_DEVICES="$GPU_ID" uv run python -m lcb_runner.dual_loop.main \
  "${COMMON_ARGS[@]}" \
  --pipeline_mode loop_b \
  --run_tag sal_patch_loop_b_only

echo "[2/2] Running pipeline ablations with SAL patch..."
