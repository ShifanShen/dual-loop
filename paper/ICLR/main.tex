...
\paragraph{Requirement alignment and multi-stage generation.}
...
\paragraph{Execution-guided repair and semantic failure analysis.}
...
\caption{Overview of the semantic-contract-based dual-loop framework. SAL scores a structured specification on coverage, faithfulness, and precision and admits field-level revisions only through the acceptance gate; the specification current at termination is frozen as contract $s^\star$ and compiled into property set $P^\star$. IRL then changes only program state, repairing under that fixed contract using public-test feedback $F_v^{(k)}$ and property-clause feedback $F_p^{(k)}$.}
\label{fig:framework}
\end{figure}
...
\medskip
\noindent\textbf{RQ1: Does the full semantic-contract pipeline improve end-to-end code generation?}
...
LPW-adapted \citep{ref_planning_driven_programming} & 0.3308 & +0.0427 \\
Full Dual-Loop & \textbf{0.3801} & \textbf{+0.0919} \\
\bottomrule
\end{tabular}
\caption{Main comparison on all 1,055 LiveCodeBench release\_v6 problems. Final Pass is the fraction of final programs passing the complete public-and-private suite; \up{} marks higher-is-better. Differences are computed from unrounded rates. External methods are shared-protocol adaptations, not reproductions.}
\label{tab:main}
\end{table}
...
\medskip
\noindent\textbf{RQ2: Does the result generalize beyond the main backbone and benchmark?}
...
\begin{tabular}{@{}lrrrl@{}}
\toprule
Setting ($n$) & Direct & Best Iter. & Full \up & Best-iterative method \\
\midrule
LCB--Qwen (50) & 0.56 & 0.70 & 0.70 & Reflexion-style \\
LCB--Llama (50) & 0.26 & 0.28 & 0.28 & Self-Refine-style \\
LCB--DeepSeek (50) & 0.30 & 0.42 & \textbf{0.50} & Self-Refine-style \\
HumanEval+--Qwen (162) & 0.8457 & 0.8889 & \textbf{0.9136} & Self-Refine-style \\
\bottomrule
\end{tabular}
\caption{Condensed cross-backbone and external validation. Final Pass rates; \up{} marks higher-is-better. The strongest controlled iterative baseline differs by setting, so the last column names the method behind each Best Iter.\ entry.}
...
\medskip
\noindent\textbf{RQ3: Which components account for the gain?}

Table~\ref{tab:mechanism} separates semantic-state from implementation movement. Average SAS rises from 80.15 to 82.29; 205 final contracts score above their initial specifications, 845 are unchanged, and 5 score lower. Across 1,219 SAL steps, 165 patches are accepted, 837 are rejected by the gate, and 217 are skipped before a patch is proposed. Across 2,061 IRL attempts, 49 reach a public-feedback pass and 36 improve the verifier signal, while 1,976 do not: 978 leave the program unchanged and 998 change it without improving it. Such a large non-improving majority is expected under a conservative gate and a fixed budget, but it bounds how much movement either loop contributes. These are state-transition counts, not causal diagnoses or evidence that SAS independently predicts correctness.
...
Signal & Observation \\
\midrule
SAS trajectory & $80.15\rightarrow82.29$ (205 up / 845 equal / 5 down) \\
SAL steps (1,219) & 165 accepted; 837 rejected; 217 skipped \\
IRL attempts (2,061) & 49 passes; 36 improved; 1,976 not improved \\
Final trace (1,055) & 401 solved / 579 spec / 4 impl. / 71 unknown \\
Solved paths (401) & 288 unchanged / 65 SAL / 38 IRL / 10 both \\
\bottomrule
\end{tabular}
\caption{Full-release mechanism and final-aligned trace evidence on 1,055 problems. Solved uses the complete-suite criterion, so the trace partition is aligned to Final Pass rather than to public feedback. Trace categories are audit cues, not causal diagnoses.}
\label{tab:mechanism}
\end{table}
...
On requirement-heavy subsets (Appendix Table~\ref{tab:subsets}), Full exceeds direct generation by 7.13--9.77 points and beats Reflexion-style repair in every category, with the largest gains on format- and edge-case-sensitive problems. Those rates come from a separate RQ-suite run, so they compare against each other rather than against Table~\ref{tab:main}.
...
\texttt{non\_negative\_output} & 152 & 4 \\
\texttt{sorted\_order} & 69 & 22 \\
\texttt{boolean\_output} & 30 & 0 \\
\texttt{same\_length} & 12 & 0 \\
\texttt{yes\_no\_output} & 12 & 0 \\
\texttt{same\_multiset} & 6 & 7 \\
\bottomrule
\end{tabular}
\caption{Property-clause coverage on LiveCodeBench release\_v6. Compiled counts are per problem; violation counts are per evaluation, so a repeatedly evaluated clause can exceed its compiled count.}
\label{tab:properties}
\end{table}

\subsection{Requirement-heavy subsets}

Table~\ref{tab:subsets} connects the mechanism analysis to requirement-heavy tasks. Full exceeds direct generation by 7.13--9.77 points on constraint-heavy, format-sensitive, edge-case-heavy, and semantic-heavy subsets and exceeds Reflexion-style repair in every category. The largest gains occur on format- and edge-case-sensitive problems, where explicit protocol and boundary obligations are especially useful. These benchmark-derived slices are not mutually exclusive causal categories.
