...
\caption{Overview of the semantic-contract-based dual-loop framework.}
\label{fig:framework}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Main comparison on all 1,055 LiveCodeBench release\_v6 problems. Final Pass is the fraction of final programs that pass the complete public-and-private suite, and \up{} marks higher-is-better. Each $\Delta$ is computed from the unrounded pass rates, so it can differ by $0.0001$ from the difference of the rounded values shown. External methods are shared-protocol adaptations, not reproductions.}
\label{tab:main}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Condensed cross-backbone and external validation, reporting Final Pass rates; \up{} marks higher-is-better. Best Iter.\ is the strongest controlled iterative baseline in each setting.}
...
\textbf{(a) Component ablation ($n=50$)}\\[2pt]
\begin{tabular}{@{}lccc@{}}
\toprule
Method & SAL & IRL & Pass \up \\
\midrule
Decomposition & no & no & 0.66 \\
SAL-only & yes & no & 0.64 \\
IRL-only & no & yes & 0.64 \\
Full & yes & yes & \textbf{0.72} \\
\bottomrule
\end{tabular}
\end{minipage}\hfill
\begin{minipage}[t]{0.54\linewidth}\centering
\textbf{(b) Requirement-heavy subsets}\\[2pt]
\begin{tabular}{@{}lrrr@{}}
\toprule
Subset & Direct & Reflexion & Full \up \\
\midrule
Constraint (337) & 0.1602 & 0.1869 & \textbf{0.2315} \\
Format (836) & 0.2512 & 0.2943 & \textbf{0.3481} \\
Edge case (389) & 0.3239 & 0.3702 & \textbf{0.4216} \\
Semantic (526) & 0.2281 & 0.2643 & \textbf{0.3213} \\
\bottomrule
\end{tabular}
\end{minipage}
\caption{Component and requirement-focused analyses. (a) A fixed 50-problem Qwen slice with three candidates per problem, distinct from the two-candidate backbone slices in Table~\ref{tab:robustness}. (b) Overlapping metadata-defined subsets of the full LiveCodeBench release\_v6 evaluation. Values are Final Pass rates.}
\label{tab:rq3}
\end{table}

Table~\ref{tab:rq3}(a) isolates the loops: SAL-only and IRL-only both reach 0.64, below Decomposition at 0.66, while Full reaches 0.72. Thus the slice supports the composition, not an isolated gain from either loop. Table~\ref{tab:rq3}(b) provides full-release evidence where semantic obligations should matter most: Full exceeds Direct by 7.13--9.77 points and Reflexion-style repair in every subset.

The transition records clarify the loops' different roles without establishing causality. Across the full release, average SAS moves from 80.15 to 82.29; SAL records 165 artifact-changing patches, while IRL records 49 passes and 36 further verifier improvements. In a historical non-compute-matched diagnostic, reopening SAL after unresolved execution failures accepted 40 revisions but only one later passed, motivating a fixed contract during repair.

\noindent\textit{Answer to RQ3.}
Neither loop alone accounts for the gain: their composition adds eight points on the controlled slice, and Full remains strongest on requirement-heavy subsets. The frozen contract connects SAL's semantic-state changes to IRL's verifier-guided repair.
...
The shared evaluation pipeline compares nine configurations. Direct NL-to-Code generates one program from $x$ without iterative feedback, and Decomposition Only induces $s^{(0)}$ and generates code with neither loop. Self-Refine-style executes public tests and iteratively revises code from that feedback \citep{ref_self_refine}, while Reflexion-style accumulates the same feedback as verbal reflection and repairs from that memory \citep{ref_reflexion}. SAL-only induces and gates revisions and then freezes the current specification without running IRL; IRL-only generates and repairs under an unrefined $s^{(0)}$. Of the external adaptations, SpecFix-BM clusters behavior from four probe programs and performs at most one requirement repair \citep{ref_specfix}, and LPW-adapted verifies and revises a plan on public examples and then holds it fixed during generation and repair \citep{ref_planning_driven_programming}; both stay within an 11-program cap. Full Dual-Loop freezes $s^\star$, selects $C^{(0)}$, and performs contract-conditioned IRL. Style-matched baselines preserve the feedback form of the cited methods, while the two external adaptations preserve their central workflows under the common model, benchmark loader, verifier, and final evaluator.

\subsection{Intermediate-representation probe}

This 50-problem Qwen probe supports representation design rather than the headline result. Because each representation adds a generation stage and budgets are not matched, it does not show that specifications outperform direct prompting; it only identifies the strongest tested explicit intermediate for later refinement and property compilation.
...
\bottomrule
\end{tabular}
\caption{Intermediate-representation probe on 50 Qwen problems, using one program per representation.}
...
Table~\ref{tab:sensitivity} reports the slice. Every threshold from 80 to 95 passes the same 20 problems. Beyond the table, the final contracts at $r=90$ and $r=95$ are identical on all 50 problems and $r=85$ differs on one; accepted revisions plateau at 15 from $r=90$, which is why we use 90.
...
Zero violations in Table~\ref{tab:properties} mean only that no evaluated output triggered the recorded check.
...
\bottomrule
\end{tabular}
\caption{Property clauses extracted from accepted contracts. Compiled counts extracted clauses; Violations counts events over verifier failures, and a clause may be evaluated more than once.}
