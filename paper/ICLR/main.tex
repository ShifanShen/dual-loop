...
Because each representation adds a generation stage and budgets are not matched, this probe does not show that specifications outperform direct prompting. It instead identifies the strongest tested explicit intermediate for the later refinement and property-compilation operations.
...
This slice uses three candidates per problem, which distinguishes it from the two-candidate cross-backbone slice. RQ3 reads the result.
...
\bottomrule
\end{tabular}
\caption{Property-clause coverage on LiveCodeBench release\_v6. Compiled counts clauses extracted from accepted contracts and Violations counts events observed over verifier failures; a clause may be evaluated more than once, so a violation count can exceed its compiled count.}
\label{tab:properties}
\end{table}

\subsection{Requirement-heavy subsets}

The slices are derived from benchmark metadata and overlap, so one problem can appear in several of them.
