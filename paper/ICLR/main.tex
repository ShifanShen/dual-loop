...
\bottomrule
\end{tabular}
\caption{Main comparison on all 1,055 LiveCodeBench release\_v6 problems. Final Pass is the fraction of final programs passing the complete public-and-private suite; \up{} marks higher-is-better. External methods are shared-protocol adaptations, not reproductions.}
\label{tab:main}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Condensed cross-backbone and external validation. Final Pass rates; \up{} marks higher-is-better. Best Iter.\ is the strongest controlled iterative baseline in each setting.}
...
On requirement-heavy subsets (Appendix Table~\ref{tab:subsets}), Full exceeds direct generation by 7.13--9.77 points and beats Reflexion-style repair in every category, with the largest gains on format- and edge-case-sensitive problems.
...
\bottomrule
\end{tabular}
\caption{Property-clause coverage on LiveCodeBench release\_v6. A clause may be evaluated more than once, so a violation count can exceed the corresponding compiled count.}
