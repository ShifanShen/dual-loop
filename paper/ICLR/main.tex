...
\bottomrule
\end{tabular}
\caption{Full-release mechanism and final-aligned trace evidence over all 1,055 problems. Solved uses the complete-suite criterion, so the trace partition aligns with Final Pass rather than with public feedback. Trace categories are audit cues, not causal diagnoses.}
\label{tab:mechanism}
\end{table}
...
\subsection{Fifty-problem probes}

Two small Qwen slices support the design choices rather than the headline result. Because each representation adds a generation stage and budgets are not matched, this probe does not show that specifications outperform direct prompting. It instead identifies the strongest tested explicit intermediate for the later refinement and property-compilation operations. The ablation slice uses three candidates per problem, which distinguishes it from the two-candidate cross-backbone slice; RQ3 reads its result.
...
\bottomrule
\end{tabular}
\subcaption{Intermediate-representation probe.}
...
\bottomrule
\end{tabular}
\subcaption{Controlled component ablation.}
\label{tab:component}
\end{minipage}
\caption{Two independent 50-problem Qwen slices. The probe in (a) uses one program per representation; the ablation in (b) uses three candidates per problem.}
\label{tab:fifty}
\end{table}
...
\subsection{Property clauses and requirement-heavy subsets}
...
\bottomrule
\end{tabular}
\subcaption{Property-clause coverage. Compiled counts clauses extracted from accepted contracts; Violations counts events over verifier failures, and a clause may be evaluated more than once.}
...
\bottomrule
\end{tabular}
\subcaption{Requirement-heavy subsets.}
\label{tab:subsets}
\end{minipage}
\caption{Contract-derived property clauses and requirement-heavy subsets on LiveCodeBench release\_v6.}
\label{tab:clauses-subsets}
\end{table}

The subsets in Table~\ref{tab:subsets} are derived from benchmark metadata and overlap, so one problem can appear in several of them.
